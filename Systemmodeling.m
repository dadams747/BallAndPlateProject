...
clc
Lt=.275;                  %length of table in meters
g=9.81;                   %gravity                    
rarm=.0254;               %radius of arm (m)
...
%Sensor Calibration
Ltc=Lt*100;  %Table length in cm
res=200 %Resolution
py=190  %x-axis position noticed by sensor
px=140  %y-axis position noticed by sensor
x=Ltc*((py/res)-.5)  %py converted to x-position noticed by system
y=Ltc*((px/res)-.5) %px converted to x-position noticed by system
